...
for dataset in datasets.glob("6560"):
    ...
    results_path = Path(".", "results_mc_kan_search_it2", dataset)
    # get the number of features
    input_size = X.shape[1]
    # define KAN architecture
    kan_archs = [[input_size, input_size - int(0.5 * input_size) + 10, 2],
    ...
    for arch in kan_archs:
        ...
        for grid in grids:
            ...
            for idx in range(10):
                ...
                # create KAN model
                model = KAN(width=arch, grid=grid, k=3, device=DEVICE)
                ...
                # dump results
                with open(result_dir.joinpath(f'kan_res_{idx}_grid_{grid}.pickle'), "wb") as output_file:
